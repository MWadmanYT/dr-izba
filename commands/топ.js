exports.run = (client, message) => {
    ...
    let number = con.query(`SELECT * FROM test ORDER by test.xp DESC`, function(err, result){
        if(err) throw err;
    ...
    con.query(`SELECT userid, xp FROM test ORDER by test.xp DESC`, function(err, top){
        if(err) throw err;
        ...
        "\n\n Ваше место - [" + number + "] Ваше XP - [" + (XP + 1) + "]```**")
        console.log('Команда "топ" использована пользователем ' + message.author.username + '. Результат - успешно');
    });
};